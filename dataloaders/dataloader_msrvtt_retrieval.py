...
class MSRVTT_DataLoader(Dataset):
    ...
    def _get_rawaudio(self, choice_audio_ids, sample_rate=16000):
        ...
        fbanks = torch.zeros((len(choice_audio_ids), target_length, 384))
        for i, audio_id in enumerate(choice_audio_ids):
            ...
            if os.path.exists(audio_path) == False:
                ...
                fbank = torchaudio.compliance.kaldi.fbank(waveform, htk_compat=True, sample_frequency=sample_rate, use_energy=False,
                                                                     window_type='hanning', num_mel_bins=384 , dither=0.0, frame_shift=f_shift)                
                # fbank = torchaudio.compliance.kaldi.fbank(waveform, htk_compat=True, sample_frequency=sample_rate, use_energy=False,
    ...
    def __getitem__(self, idx):
        ...
        # fbank = torch.zeros((len(choice_audio_ids), 1024, 128))
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, fbank, video_id, sentence
...
class MSRVTT_TrainDataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video, video_mask = self._get_rawvideo(choice_video_ids)
        fbank = self._get_rawaudio(choice_video_ids)
        # fbank = torch.zeros((len(choice_video_ids), 1024, 128))
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, fbank
...
#                     words[j] = [self.SPECIAL_TOKEN["CLS_TOKEN"]] + words[j]
#                     total_length_with_CLS = self.max_words - 1
#                     if len(words[j]) > total_length_with_CLS:
#                         words[j] = words[j][:total_length_with_CLS]
#                     words[j] = words[j] + [self.SPECIAL_TOKEN["SEP_TOKEN"]]

#                     input_ids = self.tokenizer.convert_tokens_to_ids(words[j])
#                     input_mask = [1] * len(input_ids)
#                     segment_ids = [0] * len(input_ids)
#                     while len(input_ids) < self.max_words:
#                         input_ids.append(0)
...
#                     pairs_text[j] = np.array(input_ids)
#                     pairs_mask[j] = np.array(input_mask)
#                     pairs_segment[j] = np.array(segment_ids)
...
#         # caption = self.sentences[video_id][rind]
#         # words = self.tokenizer.tokenize(caption)
#         caption = [self.sentences[video_id][i] for i in range(len(self.sentences[video_id]))]
#         words = [self.tokenizer.tokenize(caption[i]) for i in range(len(caption))]

        
#         # rind = torch.randperm(len(self.sentences[video_id]))[:10]
#         # caption = [self.sentences[video_id][rind[i]] for i in range(len(rind))]
#         # words = [self.tokenizer.tokenize(caption[i]) for i in range(len(caption))]        
#         return words
