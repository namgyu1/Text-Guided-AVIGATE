...
class ResidualAttentionBlock_Fusion(nn.Module):
    def __init__(self, d_model: int, n_head: int):
        super().__init__()
        ...
        self.attn_a = nn.MultiheadAttention(d_model, n_head)
        self.cross_attn_a = nn.MultiheadAttention(d_model, n_head)
        self.ln_a_1 = LayerNorm(d_model)
        self.mlp_a = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(d_model, d_model * 4)),
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(d_model * 4, d_model))
        ]))
        self.ln_a_2 = LayerNorm(d_model)
        self.ln_a_3 = LayerNorm(d_model)
        # self.dp = DropPath(p=0.8)
        # self.dp_a = DropPath(p=0.8)
    ...
    def attention_a(self, x: torch.Tensor, attn_mask=None):
        if attn_mask is not None:
            attn_mask_ = attn_mask.repeat_interleave(self.n_head, dim=0)
        else:
            attn_mask_ = attn_mask
        return self.attn_a(x, x, x, need_weights=False, attn_mask=attn_mask_)[0]

    def cross_attention_a(self, query: torch.Tensor, mem: torch.Tensor, attn_mask=None):
        if attn_mask is not None:
            attn_mask_ = attn_mask.repeat_interleave(self.n_head, dim=0)
        else:
            attn_mask_ = attn_mask
        return self.cross_attn_a(query, mem, mem, need_weights=False, attn_mask=attn_mask_)[0]
    ...
    def forward(self, para_tuple: tuple):
        ...
        # torch.mm(F.normalize(x.permute(1,0,2), dim=-1)[0], F.normalize(x.permute(1,0,2), dim=-1)[0].t())
        x, a, v, attn_mask= para_tuple
        if attn_mask is not None:
            video_mask = attn_mask[:, :, a.shape[0]:]
            audio_mask = attn_mask[:, :, :a.shape[0]]
        else:
            video_mask = None
            audio_mask = None

        x = x + self.attention(self.ln_1(x), attn_mask=None)
        x = x + self.cross_attention(self.ln_3(x), self.ln_3(v), video_mask)
        x = x + self.mlp_a(self.ln_a_3(x))
        # x = x + self.cross_attention(self.ln_3(x), self.ln_3(v), attn_mask.repeat(1,1,50))

        x = x + self.attention_a(self.ln_a_1(x), attn_mask=None)
        x = x + self.cross_attention_a(self.ln_a_2(x), self.ln_a_2(a), audio_mask)
        x = x + self.mlp(self.ln_2(x))

        return (x, a, v, attn_mask)
...
class Transformer_Fusion(nn.Module):
    def __init__(self, width: int, layers: int, heads: int):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_Fusion(width, heads) for _ in range(layers)])

    # def forward(self, q: torch.Tensor, v: torch.Tensor,  attn_mask=None):
    #     return self.resblocks((q, v, attn_mask))[0]

    def forward(self, q: torch.Tensor, a: torch.Tensor, v: torch.Tensor,  attn_mask=None):
        return self.resblocks((q, a, v, attn_mask))[0]
...
class ResidualAttentionBlock_Text(nn.Module):
    ...
    def cross_attention(self, query: torch.Tensor, mem: torch.Tensor, attn_mask=None):
        ...
        return self.cross_attn(query, mem, mem, need_weights=False, attn_mask=attn_mask_)[0]
        # self.cross_attn(query, mem, mem, need_weights=True, attn_mask=attn_mask_)[1].argsort(-1)[0,:,-5:]
...
class AudioVideoMHSA(nn.Module):
    def __init__(
        self,
        embed_dim,
        num_heads,
        ...
        self.attn_ = nn.MultiheadAttention(embed_dim, num_heads)
        ...
        # self.fc_1 = nn.Linear(embed_dim, embed_dim)
...
#         self.ln_2 = LayerNorm(embed_dim)
    ...
    def forward(self, x, mask, query):
        vis_att = False
        query = query.permute(1,0,2) # NLD -> LND
        x = x.permute(1,0,2) # NLD -> LND
        ori_q = query
        query = self.layer_norm1(query)
        # x = F.normalize(x, dim=-1)
        x = self.layer_norm1(x)
        # query = query.detach().cuda()
        # x = self.fc_1(x.reshape(101,12,-1,512).mean(dim=0))
        # import pdb; pdb.set_trace()
        out = self.attn_(query,x,x, need_weights=False, attn_mask=None)[0]
        ...
        #     need_weights=vis_att,
        # )
        # import pdb; pdb.set_trace()
        # out = ori_q + out
        out = self.layer_norm2(out)
        out = self.layer_norm3(out + self.dropout(self.linear_proj(out)))
