...
CKPT_NAME=ViT16_ckpt_charades_AQ_4_Transf_4_8tokens_BiMaxThsMarginAll0.1_Beta0.2_residualFuse95_CLIPVisual_gain05_woLN_CrossFirst_Aux01_TVA_5e4_2e4_0305
Tau=0.5
OMP_NUM_THREADS=48 CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --master_port $RPort --nproc_per_node=8 main_task_retrieval.py --do_train --num_thread_reader=12 \
    --epochs=5 --batch_size=128 --n_display=50 --train_csv ${DATA_PATH}/Charades_v1_train.csv --val_csv ${DATA_PATH}/Charades_v1_test.csv \
    --features_path ${DATA_PATH}/Charades_v1_480 --audio_path ${DATA_PATH}/audio_all_compressed --output_dir ckpts/${CKPT_NAME} --lr 5e-4 \
    --max_words 64 --max_frames 32 --batch_size_val 16 --datatype charades --feature_framerate 1 --coef_lr 2e-4 --freeze_layer_num 12  \
    --slice_framepos 2 --loose_type --linear_patch 2d --sim_header seqTransf --pretrained_clip_name ViT-B/16 --eval_max_frame 32 --temperature $Tau --warmup_proportion 0.1 --cross_num_hidden_layers 4 --audio_query_layers 4 --beta $beta --margin_BD $Margin --gradient_accumulation_steps 8
    #--resume_model ckpts/${CKPT_NAME}/pytorch_opt.bin.0 --init_model ckpts/${CKPT_NAME}/pytorch_model.bin.0
